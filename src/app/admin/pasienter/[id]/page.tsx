...
export default function PatientDetailsPage() {
	...
	const handleSave = async () => {
		...
		const res = await fetch(`https://fysioterapi-backend-production.up.railway.app/api/pasienter/${id}`, {
			method: "PUT",
			headers: {
	...
	return (
		<MaxWidthWrapper>
			<div className="p-8 max-w-6xl mx-auto bg-light min-h-screen">
				<h1 className="text-3xl font-bold mb-10 text-teal text-center">Pasientdetaljer for {patient?.navn}:</h1>
					...
					<div className="grid lg:grid-cols-12 gap-8">
						...
						<div className="lg:col-span-3 space-y-6">
							<div className="bg-white p-4 rounded-xl shadow border">
								<p className="text-lg font-semibold text-teal">Siste status</p>
						...
						<div className="lg:col-span-12 mt-10">
							<div className="bg-white p-6 rounded-xl shadow border">
								<h2 className="text-lg font-semibold text-teal mb-4">Anamnese</h2>

								{error && <p className="text-red-500">{error}</p>}
								{success && <p className="text-teal">{success}</p>}
								...
								<button onClick={opprettLogg} className="bg-teal text-light px-4 py-2 rounded hover:light_teal">
									Lagre
								</button>
							</div>
						</div>
						...
						<div className="lg:col-span-3 space-y-6">
							<Link href="/admin/rapporter">
								<button className="bg-light_teal text-white rounded-sm px-3 py-4 text-sm">Lag ny rapport 📝</button>
							</Link>
						</div>
						...
						<div className="lg:col-span-3 space-y-6">
							<div className="flex gap-2 justify-center">
								<DeletePatientButton patientId={patient._id} patientName={patient.navn} redirectAfterDelete={true} />
							</div>
						</div>
							...
							<div className="bg-white p-6 rounded-xl shadow border">
								...
								{reports.length > 0 ? (
									<ul className="space-y-3 text-sm text-gray-700">
										{reports.map((r) => {
											const [symptomer, observasjoner, tiltak] = r.innhold.split("\n\n");
											return (
												<li
													key={r._id}
													...
													<div className="flex justify-between items-start">
														<div>
															...
															{!open ? (
																<p className="mt-1 text-gray-500 italic truncate">{r.innhold.slice(0, 80)}...</p>
															) : (
														...
														<DeleteReportButton
															reportId={r._id}
															onDeleted={() => setReports((prev) => prev.filter((rep) => rep._id !== r._id))}
														/>
													</div>
												</li>
											);
